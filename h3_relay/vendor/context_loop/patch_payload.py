...
keyframe latents first to match the row order. On pre-PR #15439 ComfyUI it
also filters audio-only keyframes before calling the legacy method, then
restores their audio latents and invalidates the incomplete prebuilt layout so
the compatible packed-guide engine can rebuild it from the complete AV bank.
...
def _patched_extra_conds(self, **kwargs):
    ...
    audio_guides = bool(keyframes) and any(
        keyframe.get("audio_latent") is not None for keyframe in keyframes
    )
    call_kwargs = kwargs
    if audio_guides:
        # Pre-PR #15439 extra_conds assumes every keyframe has video and raises
        # before this wrapper can repair the payload. Let it process only the
        # video-bearing subset, then restore the complete AV guide bank below.
        call_kwargs = dict(kwargs)
        call_kwargs["minimax_keyframes"] = [
            keyframe for keyframe in keyframes
            if keyframe.get("latent") is not None
        ]
    out = _orig_extra_conds(self, **call_kwargs)

    marked_merge = bool(keyframes) and bool(refs) and (
        any(MC_KEY in keyframe for keyframe in keyframes)
        or any(MC_AUDIO_KEY in ref for ref in refs)
    )
    if not audio_guides and not marked_merge:
    ...
    if audio_guides:
        ...
        # Stock prebuilt this layout from the filtered video-only list. The
        # patched model engine rebuilds it once from the restored AV bank.
